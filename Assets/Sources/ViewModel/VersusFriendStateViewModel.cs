...
{
    public class VersusFriendStateViewModel : IHandler<StateUpdated>
    {
        ...
        public VersusFriendStateViewModel(IScreensFacade screens, IModeContext modeContext, RollConfig config)
        {
        ...
        {
            if (handle.Container.Count != 2 || _modeContext.Selected is not VersusFriendMode mode)
                return;
            ...
            if (handle.JustRolled)
            {
                Screen.CubesView.AnimateCubes(handle.Mover.Roller.Values, _config.GetProfile(profile),
                    delegate
                    {
                        ...
                        Screen.GetDuoCombinatorView(playerIndex).SetAllInteractable(false);
                    },
                    delegate
                    {
                        Screen.SetRollInteractable(handle.Mover.RemainRolls > 0);
                        
                        Screen.GetDuoCombinatorView(playerIndex).SetAllInteractable(true);
        ...
        private void UpdateSubscriptions(StateUpdated state)
        {
            ...
            bool firstPlayer = state.Mover == state.Container[0];
            ...
            if (firstPlayer)
            {
                ...
                if (_last != null)
                    Screen.GetDuoCombinatorView(1).OnClicked -= _modeContext.Selected.RegisterCombination;
            }
            else
            {
                Screen.GetDuoCombinatorView(1).OnClicked += _modeContext.Selected.RegisterCombination;
